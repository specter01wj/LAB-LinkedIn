...
protocol CartDelegate {
  ...
  func calculateSubtotalForLineItemableItems(items: [LineItemable]) -> Double
  
}

struct Calculator: CartDelegate {
  
  func calculateSubtotalForLineItemableItems(items: [LineItemable]) -> Double {
    guard items.count > 0 else { return 0.00 }
...
class Cart {
  ...
  func updateSubtotal() {
    if let delegate = delegate {
      print("Subtotal: $\(delegate.calculateSubtotalForLineItemableItems(items))")
    }
  }
  
  func addItem(item: LineItemable) {
    items.append(item)
    updateSubtotal()
  }
...
let shoppingCart = Cart(delegate: Calculator())
shoppingCart.addItem(book)
shoppingCart.addItem(bluRay)
